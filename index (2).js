...
const commands = [
    ...
    new SlashCommandBuilder()
        ...
        .addUserOption(opt =>
            opt.setName('utilisateur').setDescription('Le membre à signaler').setRequired(true))
        .addStringOption(opt =>
            opt.setName('raison').setDescription('Raison du signalement').setRequired(true)
                .addChoices(
        ...
        .addStringOption(opt =>
            opt.setName('commentaire').setDescription('Détails supplémentaires (optionnel)').setRequired(false))
        .toJSON(),
    ...
    new SlashCommandBuilder()
        ...
        .addBooleanOption(opt =>
            opt.setName('visible').setDescription('Rendre visible pour tout le monde ?').setRequired(false))
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
        .toJSON(),

    // /stats (importée depuis stats.js)
    statsCommand
];
...
client.once('ready', async () => {
    ...
    try {
        ...
        console.log('✅ Commandes enregistrées : /signal /supsignal /adminsignal /stats');
    } catch (err) {
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'adminsignal') {
        ...
        signaux.forEach(s => {
            ...
            if (s.commentaire && s.commentaire !== 'Aucun commentaire')
                compteur[s.reportedId].commentaires.push(
                    `*"${s.commentaire}"* — par **${s.reporterName}**`
                );
        });
        ...
        const tousCommentaires = sorted
            .flatMap(([id, info]) => info.commentaires.map(c => `<@${id}> : ${c}`))
            .slice(0, 15);
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '💬 Commentaires', value: tousCommentaires.length > 0 ? tousCommentaires.join('\n') : 'Aucun' }
            )
            .setColor(0xE74C3C)
            .setTimestamp()
